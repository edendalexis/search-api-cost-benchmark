...
export const PROVIDERS = {
  ...
  // Exa's second documented content mode. `text` returns whole pages; `highlights`
  // runs a model over each page and returns the passages it judges relevant to the
  // query, capped at Exa's own 4,000 characters. Measured before adding it: the
  // returned length varies with the page (554 chars from a 2,543-char page, 794 from
  // 1,829, 4,000 from 14,066), so it is an extraction and not a truncation, which is
  // what makes it a mode rather than a setting we chose.
  'exa-highlights': {
    vendor: 'Exa',
    envKey: 'EXA_API_KEY',
...
const SERVER_MS = {
  ...
  'exa-highlights': (j) => (typeof j.searchTime === 'number' ? j.searchTime : null),
};
