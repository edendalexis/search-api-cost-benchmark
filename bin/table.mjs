...
const arg = (n, d) =>
  (process.argv.find((a) => a.startsWith(`--${n}=`)) || `--${n}=${d}`).slice(
    n.length + 3,
  );
...
priced.sort(
  SORT === "quality"
    ? (a, b) =>
        b.correct_rate - a.correct_rate ||
        a.total_usd_per_1k - b.total_usd_per_1k
...
const rows = priced.map((a) => {
  ...
  return {
    ...a,
    vendor,
    tier: a.arm.replace(/^[a-z]+-/, ""),
    domain: DOMAIN[vendor] || "",
    hue: HUE[vendor] || "#8aa0a7",
  };
});
...
// WHAT SURVIVES BEING COMPARED ON BOTH AXES AT ONCE. An arm is dominated when another
// arm in the same run is cheaper AND scored higher: there is no reading of the table on
// which you would pick it. What is left is the frontier.
//
// This is the only honest way to emphasise rows in a benchmark the maintainer competes
// in: the rule is applied to everyone and decides for itself who survives. On the
// published run it keeps two of the maintainer's arms, keeps two competitors, and
// eliminates the maintainer's most expensive arm. A highlight you cannot lose is not a
// finding, it is decoration.
const beatenBy = (a, all) =>
  all.filter(
    (b) =>
      b.total_usd_per_1k < a.total_usd_per_1k &&
      b.correct_rate > a.correct_rate,
  );
const FRONTIER = process.argv.includes("--frontier");
for (const a of rows) a.beaten = beatenBy(a, rows).map((b) => b.arm);
...
const md = () => {
  ...
  const body = rows.map(
    (a, i) =>
      `| ${i + 1} | \`${a.arm}\` | ${usd(a.api_usd_per_1k)} | ${num(a.tokens_per_query)} | ${usd(a.llm_usd_per_1k)} | **${usd(a.total_usd_per_1k)}** | ${a.correct_rate}% | ${usd(a.usd_per_correct, 4)} |`,
  );
  const notes = [
    ...free.map(
      (a) =>
        `\`${a.arm}\` costs zero at the door on every plan of its vendor, free or paid. That is its list price, not a promotion, and the token column still applies.`,
    ),
    ...unpriced.map(
      (a) => `\`${a.arm}\` is unranked: ${a.api_basis}. Unranked is not free.`,
    ),
  ];
  return `${head}\n${body.join("\n")}${notes.length ? `\n\n${notes.join("  \n")}` : ""}`;
};
...
const providerList = async () => {
  ...
  const line = (v) => {
    const site = DOMAIN[v.vendor]
      ? `[${v.vendor}](https://${DOMAIN[v.vendor]})`
      : v.vendor;
    const links = [
      v.docs && `[docs](${v.docs})`,
      v.pricing && `[pricing](${v.pricing})`,
    ]
      .filter(Boolean)
      .join(" &middot; ");
...
const X_QBAR = W - PAD - 34,
  X_CORRECT = X_QBAR - 8,
  X_TOTAL = X_CORRECT - 48,
  X_TOK = X_TOTAL - 88;
...
const svg = (t) => {
  const H =
    ...
    rows.length * ROW +
    26 +
    17 * ((FRONTIER ? 3 : 2) + free.length + unpriced.length);
  const o = [];
  o.push(
    `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" font-family="${MONO}">`,
  );
  o.push(`<rect width="${W}" height="${H}" fill="${t.ground}"/>`);
  o.push(
    `<defs><clipPath id="sq"><rect width="26" height="26" rx="6"/></clipPath></defs>`,
  );
  ...
  const leg = [
    ...
    [
      t.tok,
      `tokens, read on ${R.answering_model.model.split("/")[1]} at $${R.answering_model.usd_per_mtok_input}/M`,
    ],
    [t.good, "correct"],
  ];
  let lx = PAD;
  for (const [c, s] of leg) {
    o.push(
      `<rect x="${lx}" y="18" width="18" height="8" rx="1.5" fill="${c}"/>`,
    );
    o.push(
      `<text x="${lx + 25}" y="26" font-size="10.5" fill="${t.faint}" letter-spacing="0.5">${esc(s)}</text>`,
    );
    lx += 25 + textW(s, 10.5) + 24;
  }
  o.push(
    `<text x="${W - PAD}" y="26" font-size="10.5" fill="${t.faint}" text-anchor="end" letter-spacing="0.5">axis capped at $${CAP}</text>`,
  );

  // header
  const th = (x, s, anchor = "end") =>
    `<text x="${x}" y="${HEAD - 14}" font-size="9.5" fill="${t.faint}" text-anchor="${anchor}" letter-spacing="1.25">${esc(s.toUpperCase())}</text>`;
  ...
  o.push(
    `<line x1="${PAD}" y1="${HEAD - 7}" x2="${W - PAD}" y2="${HEAD - 7}" stroke="${t.line}"/>`,
  );
  ...
  rows.forEach((a, i) => {
    ...
    if (i % 2)
      o.push(
        `<rect x="0" y="${y}" width="${W}" height="${ROW}" fill="${t.alt}"/>`,
      );
    o.push(
      `<line x1="${PAD}" y1="${y + ROW}" x2="${W - PAD}" y2="${y + ROW}" stroke="${t.line}" stroke-opacity="0.5"/>`,
    );
    // A dominated row is dimmed rather than removed: the claim is that it lost a
    // comparison, not that it does not exist.
    const dead = FRONTIER && a.beaten.length > 0;
    if (FRONTIER && !dead)
      o.push(
        `<rect x="0" y="${y + 3}" width="3" height="${ROW - 6}" fill="${t.good}"/>`,
      );
    o.push(`<g${dead ? ' opacity="0.34"' : ""}>`);

    // brand mark on the vendor's own hue, then vendor and tier
    const src = mark(a.domain);
    o.push(`<g transform="translate(${PAD} ${mid - 13})">`);
    o.push(
      `<rect width="26" height="26" rx="6" fill="${src ? "none" : a.hue}"/>`,
    );
    if (src)
      o.push(
      ...
      o.push(
        `<text x="13" y="18" font-size="13" font-weight="700" fill="#0b0f11" text-anchor="middle">${esc(a.vendor[0])}</text>`,
      );
    o.push("</g>");
    o.push(
      `<text x="${PAD + 37}" y="${mid + 5}" font-size="13.5" fill="${t.ink}">${esc(a.vendor)} <tspan fill="${t.dim}">${esc(a.tier)}</tspan></text>`,
    );

    // the bar: price at the door, then the tokens the payload pushes into the model
    const scale = TRACK_W / CAP;
    const apiW = Math.max(
      a.api_usd_per_1k * scale,
      a.api_usd_per_1k > 0 ? 2 : 0,
    );
    ...
    if (apiW > 0)
      o.push(
        `<rect x="${TRACK_X}" y="${by}" width="${apiW.toFixed(1)}" height="${bh}" rx="2.5" fill="${t.api}"/>`,
      );
    o.push(
      `<rect x="${(TRACK_X + apiW).toFixed(1)}" y="${by}" width="${tokW.toFixed(1)}" height="${bh}" rx="2.5" fill="${over ? t.hot : t.tok}"/>`,
    );
    if (over) {
      o.push(
        `<rect x="${TRACK_X + TRACK_W - 11}" y="${by - 3}" width="13" height="${bh + 6}" fill="${t.hot}"/>`,
      );
      o.push(
        `<rect x="${TRACK_X + TRACK_W - 6}" y="${by - 3}" width="4" height="${bh + 6}" fill="${t.ground}"/>`,
      );
    }
    ...
    if (inA) o.push(inA);
      ...
      o.push(
        `<text x="${TRACK_X - 8}" y="${mid + 4}" font-size="11" font-weight="700" fill="${t.api}" text-anchor="end">${esc(aLab)}</text>`,
      );
    ...
    if (inT) o.push(inT);
      ...
      o.push(
        `<text x="${(TRACK_X + apiW + tokW + 8).toFixed(1)}" y="${mid + 4}" font-size="11" font-weight="700" fill="${over ? t.hot : t.tok}">${esc(tLab)}</text>`,
      );

    o.push(
      `<text x="${X_TOK}" y="${mid + 5}" font-size="13" fill="${t.dim}" text-anchor="end">${num(a.tokens_per_query)}</text>`,
    );
    o.push(
      `<text x="${X_TOTAL}" y="${mid + 5}" font-size="15" font-weight="700" fill="${t.ink}" text-anchor="end">${usd(a.total_usd_per_1k)}</text>`,
    );
    o.push(
      `<text x="${X_CORRECT}" y="${mid + 5}" font-size="13" font-weight="700" fill="${t.ink}" text-anchor="end">${a.correct_rate}%</text>`,
    );
    o.push(
      `<rect x="${X_QBAR}" y="${mid - 2}" width="34" height="4" rx="2" fill="${t.line}"/>`,
    );
    o.push(
      `<rect x="${X_QBAR}" y="${mid - 2}" width="${((34 * a.correct_rate) / 100).toFixed(1)}" height="4" rx="2" fill="${t.good}"/>`,
    );
    o.push("</g>");
  });

  const fy = HEAD + rows.length * ROW + 24;
  const nDead = rows.filter((a) => a.beaten.length).length;
  const foot = [
    ...(FRONTIER
      ? [
          `Highlighted: no other configuration in this run is both cheaper and scored higher. ${nDead} of ${rows.length} are, including one of the maintainer's own.`,
        ]
      : []),
    ...
    ...free.map(
      (a) =>
        `${a.arm} costs zero at the door on every plan of its vendor. That is its list price, not a promotion.`,
    ),
    ...unpriced.map(
      (a) => `${a.arm} is unranked: ${a.api_basis}. Unranked is not free.`,
    ),
  ...
  foot.forEach((s, i) => {
    ...
    o.push(
      `<text x="${PAD}" y="${fy + i * 17}" font-size="${size.toFixed(2)}" fill="${t.faint}">${esc(s)}</text>`,
    );
  });
  o.push("</svg>");
  return o.join("\n");
...
const stem = FRONTIER ? "table-frontier" : "table";
writeFileSync(`${ROOT}docs/${stem}-dark.svg`, `${svg(THEMES.dark)}\n`);
writeFileSync(`${ROOT}docs/${stem}-light.svg`, `${svg(THEMES.light)}\n`);

// --frontier produces a variant of the chart only. It must not touch the README or the
// run's report: the published table stays the plain one, ranked and unannotated.
const table = md();
if (!FRONTIER) {
  writeFileSync(
    `${ROOT}${RUNREL}/report.md`,
  ...
  if (existsSync(readme)) {
    ...
    } else if (!/<!-- RESULTS -->/.test(src))
      console.log("README has no <!-- RESULTS --> markers; nothing injected");
  }
}
console.log(
  `docs/${stem}-dark.svg, docs/${stem}-light.svg${FRONTIER ? "" : `, ${RUNREL}/report.md`}`,
);
console.log(
  `${rows.length} arms ranked by ${SORT}${unpriced.length ? `, ${unpriced.length} unpriced and unranked` : ""}`,
);
